...
#Also 1 LUAD and 1 HNSCC cases are missing in the file with mutational signatures annotations, so need to exclude them:
ids_map=ids_map[!(ids_map$CASE_ID %in% c('C3L-00083','C3N-03612')),]


#Read the list of protein-coding genes:
gene_list=read.table('gencode_gene_info_v22/gencode.gene.info.v22.tsv',sep='\t',header=T)
...
mod_kclus=ifelse(mod_kclus==1, 'Smoking_high','Smoking_low')
