...
column_ha = HeatmapAnnotation(MSI_score=col_annot$MSI_score,MSI_status=col_annot$MSI_status,MutCount = anno_barplot(col_annot$Mut_count,bar_width = 0.5, height = unit(1, "cm")), Germline_somatic_MMR_genes=anno_simple(col_annot$empty,col=c("NA"="white"),pch=18,pt_gp = gpar(col = ifelse(col_annot$Germline==1,"#984ea3","#4daf4a")),pt_size = unit(ifelse(col_annot$Germline==1 | col_annot$Somatic==1,4, NA), "mm")), 
MMR_MutSign = anno_points(log2(col_annot$Count+1), 
#        gp = gpar(col = ifelse( col_annot$Signature_annot=='High_candidate',"red",ifelse( col_annot$Case %in% outliers,"blue","black"))), 
        gp = gpar(col = ifelse(col_annot$Count_MSI_MutSignature >=100,"red",ifelse(col_annot$Case %in% outliers,"blue","black"))), 
        height = unit(1.5, "cm")),
col = list(GermlineMMRGenes = c("1" = "#990000", "0" = "white"), SomaticMMRGenes = c("1" = "black", "0" = "white"), SomCNV_HRDcore=c("1" = "grey", "0" = "white"), MSI_status=c("MSI-High" = "#e34a33", "MSS" = "#fee8c8"),MSI_score=colorRamp2(c(0,10,20,30,40), c("#ffffcc","#fed976", "#fd8d3c","#e31a1c", "#800026"))))

split_row=factor(row_annot$Marker_type, levels=c('Gene_expression','Protein_expression','Phosphosite_expression'))
split_column=factor(col_annot$Subtype_by_markers,levels=c('MSI_low','MSI_high'))
#split_column=factor(col_annot$Signature_annot,levels=c('Other','High_candidate'))


genes_annot=row_annot$Gene %in% c(c_genes$V1,"RPL22L1") 
...
print(ht)
for (slice in 1:2){
for(an in c('MSI_score','MSI_status','Germline_somatic_MMR_genes')) {
