...
#Read PAM50 gene list:
pam50_g=read.table('DATA/PAM50/PAM50_from_BRCA_paper_S3.txt',sep='\t',header=F)
pam50_g=pam50_g[pam50_g$V2 %in% ids_map$Proteome_Sample_ID,]
colnames(pam50_g)=c('PAM50','Protein_sample_ID')
rownames(pam50_g)=pam50_g$Protein_sample_ID

pam50_g=pam50_g[ids_map$Proteome_Sample_ID,]
pam50_g$CASE_ID=ids_map$CASE_ID



hrd_genes=c('BRCA1','BRCA2')
...
g_path=g_path[g_path$Sample %in% ids_map$CASE_ID & g_path$HUGO_Symbol %in% hrd_genes & g_path$Overall_Classification!='Prioritized VUS' & g_path$Disease==can,]
...
som_s=som_s[som_s$Sample %in% ids_map$CASE_ID & som_s$Hugo_Symbol %in% hrd_genes & som_s$COHORT==can,]


#Add annotations of germline and somatic mutations in the HRD-associated genes (BRCA1 and BRCA2).
sign$Germline=ifelse(sign$Case %in% g_path$Sample, 1, 0)
sign$Somatic=ifelse(sign$Case %in% som_s$Sample, 1, 0)

pam50_g_s=pam50_g[,c('CASE_ID','PAM50')]
colnames(pam50_g_s)[1]='Case'
sign=merge(sign,pam50_g_s)

#Read HRD-score table:
hrd_s=read.table(paste('DATA/HRD_score/',can,'_HRDscores.tsv',sep=''),sep='\t',header=T)
hrd_s=hrd_s[,c('sample_ID','HRD_score')]
colnames(hrd_s)[1]='Case'
hrd_s$Case<-gsub('CPT000814','604',hrd_s$Case)
hrd_s$Case<-gsub('CPT001846','01BR044',hrd_s$Case)
hrd_s=hrd_s[!duplicated(hrd_s),]

sign=merge(sign,hrd_s)
...
path_f=paste('DATA/Expression_data/Proteome/Proteome_Broad_updated_tumor_NAT_raw_gene_level.tsv.gz',
sep='')
...
mod_kclus=ifelse(mod_kclus==1, 'HRD_high','HRD_low')
