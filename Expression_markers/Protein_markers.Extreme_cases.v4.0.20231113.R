...
path_f=paste('/diskmnt/Projects/TSAP/data/harmonized_data_freeze/Expression_data/Proteome/Proteome_Broad_updated_tumor_NAT_raw_gene_level.tsv.gz',
sep='')

ph=fread(path_f)
ph=as.data.frame(ph)

#We can keep using geneSymbol, as they are all unique in this table
#length(unique(ph$geneSymbol))==nrow(ph) -- TRUE

#Also proteome ans phosphoproteome has the same Sample IDs:
#nrow(ids_map[ids_map$Proteome_Sample_ID==ids_map$Phosphoproteome_Sample_ID,])==nrow(ids_map)
...
write.table(tab, 'inputs/Protein_expr_perSample.20231113.tsv',sep='\t',quote=F,row.names=F)
tab=read.table('inputs/Protein_expr_perSample.20231113.tsv',sep='\t',header=T)
...
all_unique_ids=unique(tab_3$geneSymbol)
...
all_st<-foreach(ids_set_n=c(1:40)) %dopar% {
    ...
    tab_4=tab_3[tab_3$geneSymbol %in% sel_ids,]

all_st_per_core=NULL
    for (id in unique(tab_4$geneSymbol)){
    	tab_s=tab_4[tab_4$geneSymbol==id,]
...
write.table(all_test_stat, 'out/Protein_markers.20231204.tsv',sep='\t',quote=F,row.names=F)
